...
def test_sync_reports_counts(tmp_path, monkeypatch, capsys) -> None:
    ...
    _create_milestone(tmp_path / ".plan" / "milestones" / "stage-2")
...
def _create_milestone(directory: Path, issue_names: tuple[str, ...] = ()) -> None:
    directory.mkdir(parents=True, exist_ok=True)
    (directory / "milestone.md").write_text("# Milestone\n", encoding="utf-8")
    if issue_names:
        issues_dir = directory / "issues"
        issues_dir.mkdir(parents=True, exist_ok=True)
        for name in issue_names:
            (issues_dir / name).write_text("# Issue\n", encoding="utf-8")
